...
class WC_Gateway_Afterpay_Nl_Payinx extends WC_Gateway_Afterpay_Base_Rest {
	...
	public function __construct() {
		...
		$this->id           = 'afterpay_nl_payinx';
		$this->method_title = __( 'Riverty Netherlands Pay In 3', 'afterpay-payment-gateway-for-woocommerce' );
		$this->has_fields   = true;

		// Setup field to load pay in x details in.
		$this->available_payinx_information = array();
		...
		if ( 'yes' === $this->enabled ) {
			add_action( 'wp_enqueue_scripts', array( &$this, 'init_payinx_element_scripts' ) );
		}
	}
	...
	public function init_form_fields() {
		$this->form_fields = apply_filters(
			'afterpay_nl_payinx_form_fields', array(
				'enabled'                               => array(
					'title'   => __( 'Enable/Disable', 'afterpay-payment-gateway-for-woocommerce' ),
					'type'    => 'checkbox',
					'label'   => __( 'Enable Riverty Netherlands Pay In 3', 'afterpay-payment-gateway-for-woocommerce' ),
					'default' => 'no',
				),
				'test_api_key'                               => array(
	...
	public function payment_fields() {
		...
		<?php echo $this->get_payinx_element(); ?>

		<fieldset class="riverty_fieldset">
			<?php if ( $this->can_show_introduction_text() ) : ?>
				<?php echo $this->get_introduction_text(); ?>
			<?php endif; ?>
			<input type="hidden" name="<?php echo esc_attr( $this->id ); ?>_due_amount" id="<?php echo esc_attr( $this->id ); ?>_due_amount" value="<?php echo array_column( $this->available_payinx_information, 'dueAmount' )[0]; ?>"/>
