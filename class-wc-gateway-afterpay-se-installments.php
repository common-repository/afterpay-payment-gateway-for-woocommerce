<?php
/**
 * AfterPay Installments payment method for Sweden
 *
 * @category   Class
 ...
 * AfterPay Installments payment method for Sweden
 *
 * @class         WC_Gateway_Afterpay_Se_Installments
 * @extends       WC_Gateway_Afterpay_Base_Rest
 * @package       Arvato_AfterPay
 * @author        AfterPay
 */
#[AllowDynamicProperties]
class WC_Gateway_Afterpay_Se_Installments extends WC_Gateway_Afterpay_Base_Rest {
	...
	public function __construct() {
		...
		$this->id           = 'afterpay_se_installments';
		$this->method_title = __( 'Riverty Sweden Fixed instalments', 'afterpay-payment-gateway-for-woocommerce' );
		...
		$this->title             = 'Riverty Fix – Fast betalning varje månad';
		$this->extra_information = 'Samma belopp varje månad.';
	...
	public function init_form_fields() {
		...
		$this->form_fields = apply_filters(
			'afterpay_se_installments_form_fields', array(
				'enabled'                               => array(
					'title'   => __( 'Enable/Disable', 'afterpay-payment-gateway-for-woocommerce' ),
					'type'    => 'checkbox',
					'label'   => __( 'Enable Riverty Sweden Fixed instalments', 'afterpay-payment-gateway-for-woocommerce' ),
					'default' => 'no',
				),
				'test_api_key'                               => array(
	...
	public function payment_fields() {
		...
		<fieldset class="riverty_fieldset">
			<p class="form-row">
				<label for="afterpay_installmentplan"><strong>Fasta avbetalningar</strong></label>
				<?php 
					$i = $installment_plan_initial_amount = 0; 
					function sort_installment_plans($first_installment_plan, $second_installment_plan) {
						if ($first_installment_plan->numberOfInstallments == $second_installment_plan->numberOfInstallments) {
							return 0;
						}
						return ($first_installment_plan->numberOfInstallments < $second_installment_plan->numberOfInstallments) ? -1 : 1;
					}
					usort($this->available_installment_plans, "sort_installment_plans");
				...
				<?php 
					if( $installment_plan_initial_amount == $installment_plan->installmentAmount ) {
						continue;
					}
					$installment_plan_initial_amount = $installment_plan->installmentAmount; 				
				?>
				...
				<div class="installment_plan 
					...
					?>
					" id="installment_plan_<?php echo esc_attr( $installment_plan->installmentProfileNumber ); ?>">
					<p class="afterpay_terms_and_conditions">
						Då kredittagaren handlar för <?php echo esc_html( $installment_plan->basketAmount ); ?> kr och väljer att dela upp betalningen på <?php echo esc_html( $installment_plan->numberOfInstallments ); ?> månader kommer den effektiva årsräntan att uppgå till <?php echo esc_html( $installment_plan->effectiveAnnualPercentageRate ); ?> %. Det totala beloppet att betala blir i detta exempel <?php echo esc_html( $installment_plan->totalAmount ); ?> kr.
						<?php echo str_replace("%s", esc_html( $installment_plan->readMore ), __( 'Here you will find the <a class="afterpay_terms_link" href="%s" target="_blank">European Standard Information for Consumer Credit</a>.', 'afterpay-payment-gateway-for-woocommerce' ) ); ?>
					</p>
				</div>
				<?php $i++; ?>
				<?php endforeach; ?>
		...
		<fieldset class="riverty_fieldset">
			...
			<p class="form-row validate-required validate-phone">
				<label for="<?php echo esc_attr( $this->id ); ?>_phone"><strong><?php echo esc_html__( 'Phone number', 'afterpay-payment-gateway-for-woocommerce' ); ?>: <span class="required">*</span></strong></label>
