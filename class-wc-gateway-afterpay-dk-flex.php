<?php
/**
 * AfterPay Flex payment method for Denmark
 *
 * @category   Class
 ...
 * AfterPay Flex payment method for Denmark
 *
 * @class         WC_Gateway_Afterpay_Dk_Flex
 * @extends       WC_Gateway_Afterpay_Base_Rest
 * @package       Arvato_AfterPay
 * @author        AfterPay
 */
#[AllowDynamicProperties]
class WC_Gateway_Afterpay_Dk_Flex extends WC_Gateway_Afterpay_Base_Rest {
	...
	public function __construct() {
		...
		$this->id           = 'afterpay_dk_flex';
		$this->method_title = __( 'Riverty Denmark Flex payment', 'afterpay-payment-gateway-for-woocommerce' );
		...
		$this->title             = 'Riverty Flex - Betal i egen takt';
		$this->extra_information = 'Læg flere køb sammen på en konto';
	...
	public function init_form_fields() {
		...
		$this->form_fields = apply_filters(
			'afterpay_dk_flex_form_fields', array(
				'enabled'                               => array(
					'title'   => __( 'Enable/Disable', 'afterpay-payment-gateway-for-woocommerce' ),
					'type'    => 'checkbox',
					'label'   => __( 'Enable Riverty Denmark Flex payment', 'afterpay-payment-gateway-for-woocommerce' ),
					'default' => 'no',
				),
				'test_api_key'                               => array(
	...
	public function payment_fields() {
		...
		<fieldset class="riverty_fieldset">
			...
			<?php endif; ?>
			<p class="form-row">
				<p>Månedsbeløb: <?php echo esc_html( round( $this->available_flex_information['installmentAmount'], 2 ) ); ?> kr eller ca 10 % av kredittbeløbet</p>
				<ul class="afterpay_installment_bullets" style="margin-top: 10px;">
					<li style="list-style-type: circle !important;">Ingen opstartsbeløb</li>
					<li style="list-style-type: circle !important;">Månedsgebyr: <?php echo esc_html( number_format( $this->available_flex_information['monthlyFee'], 2 ) ); ?> kr / kk</li>
					<li style="list-style-type: circle !important;">Årlig rente: <?php echo esc_html( $this->available_flex_information['interestRate'] ); ?> %</li>
				</ul>
				<p>Ansøgning om afdrag må gennemføres etter at købet er gjort.</p>
				<br/>
				<p>Eksempel: Hvis et køb på 1000 kr tilbagebetales over 12 måneder, udgør de samlede omkostninger for købet 1085 kr med en årlig omkostningsprocent(ÅOP) på 28,02% før skat. Læs <a href="<?php echo $this->available_flex_information['readMore']; ?>" target="blank">Standard European Consumer Credit Information</a></p>
				<br/>
				<input type="hidden" name="<?php echo esc_attr( $this->id ); ?>_flex_profile" value="<?php echo $this->available_flex_information['profileNo']; ?>"/>
			</p>
			<div class="clear"></div>
			<p class="form-row">
				<label for="<?php echo esc_attr( $this->id ); ?>_ssn"><strong>Tast inn dit CPR-nummer (10 siffer) for å gennemføre købet: <span class="required">*</span></strong></label>
