<?php
/**
 * AfterPay B2B invoice payment method for Denmark
 *
 * @category   Class
 ...
 * AfterPay B2B invoice payment method for Denmark
 *
 * @class         WC_Gateway_Afterpay_Dk_B2B_Openinvoice
 * @extends       WC_Gateway_Afterpay_Base_Rest
 * @package       Arvato_AfterPay
 * @author        AfterPay
 */
#[AllowDynamicProperties]
class WC_Gateway_Afterpay_Dk_B2B_Openinvoice extends WC_Gateway_Afterpay_Base_Rest {
	...
	public function __construct() {
		...
		$this->id           = 'afterpay_dk_b2b_openinvoice';
		$this->method_title = __( 'Riverty Denmark B2B invoice', 'afterpay-payment-gateway-for-woocommerce' );
		...
		$this->title             = 'Faktura til virksomheder';
		$this->extra_information = 'Køb nu, betal om 30 dage';
	...
	public function init_form_fields() {
		...
		$this->form_fields = apply_filters(
			'afterpay_dk_b2b_openinvoice_form_fields', array(
				'enabled'                               => array(
					'title'   => __( 'Enable/Disable', 'afterpay-payment-gateway-for-woocommerce' ),
					'type'    => 'checkbox',
					'label'   => __( 'Enable Riverty Denmark B2B invoice', 'afterpay-payment-gateway-for-woocommerce' ),
					'default' => 'no',
				),
				'test_api_key'                               => array(
	...
	public function payment_fields() {
		...
		<fieldset class="riverty_fieldset">
			...
			<p class="form-row validate-required">
				<label for="<?php esc_html_e( $this->id ); ?>_companyname"><strong>Virksomhedens navn: <span class="required">*</span></strong></label>
				<input type="input" class="input-text" name="<?php esc_html_e( $this->id ); ?>_companyname" />
			</p>
			<p class="form-row validate-required">
				<label for="<?php esc_html_e( $this->id ); ?>_cocnumber"><strong>CVO nbr: <span class="required">*</span></strong></label>
	...
	public function validate_fields() {
		...
		if ( empty( $_POST[ $this->id . '_cocnumber' ] ) ) {
			array_push( $validation_messages, __( 'Vat number is a required field', 'afterpay-payment-gateway-for-woocommerce' ) );
		}
